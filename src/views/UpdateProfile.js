...
function StudentProfile() {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch("http://localhost:3000/c/profile", {
        method: "GET",
        headers: {
      ...
      setFormData({
        CNAME: data.CNAME || "",
        CITY: data.CITY || "",
        CPROFILE: data.CPROFILE || "",
        CREGDATE: data.CREGDATE || "",
      });
  ...
  const handleFormSubmit = async (event) => {
    ...
    try {
      const response = await fetch("http://localhost:3000/c/updateprofile", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${localStorage.getItem("token")}`,
